...
public class RestWordFeudClient implements WordFeudClient {
     ...
     */
    @Override
    public Invite invite(final String username, final RuleSet ruleset, final BoardType boardType) {
        ...
        JSONObject jsonObject = callAPI(path, toJSON(parameters));

        try {
            JSONObject content = jsonObject.getJSONObject("content");
            String invitation = content.getString("invitation");
            return Invite.fromJson(invitation);
        } catch (JSONException e) {
            throw new RuntimeException("Could not deserialize JSON", e);
        }
